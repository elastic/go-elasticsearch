...
func newProjectDeleteRoutingFunc(t Transport) ProjectDeleteRouting {
	return func(name string, o ...func(*ProjectDeleteRoutingRequest)) (*Response, error) {
		var r = ProjectDeleteRoutingRequest{Name: name}
		for _, f := range o {
			f(&r)
		}
...
// ProjectDeleteRouting delete named project routing expression
//
...
// ProjectDeleteRoutingRequest configures the Project Delete Routing API request.
...
func (r ProjectDeleteRoutingRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.BeforeRequest(req, "project.delete_routing")
	}
	res, err := transport.Perform(req)
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "project.delete_routing")
	}
	if err != nil {
		if instrument, ok := r.Instrument.(Instrumentation); ok {
			instrument.RecordError(ctx, err)
...
func (f ProjectDeleteRouting) WithContext(v context.Context) func(*ProjectDeleteRoutingRequest) {
	return func(r *ProjectDeleteRoutingRequest) {
		r.ctx = v
	}
}
