...
func newReindexListFunc(t Transport) ReindexList {
	return func(o ...func(*ReindexListRequest)) (*Response, error) {
		var r = ReindexListRequest{}
		for _, f := range o {
			f(&r)
		}
...
func (r ReindexListRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		ctx = instrument.Start(providedCtx, "reindex_list")
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.BeforeRequest(req, "reindex_list")
	}
	res, err := transport.Perform(req)
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "reindex_list")
	}
	if err != nil {
		if instrument, ok := r.Instrument.(Instrumentation); ok {
			instrument.RecordError(ctx, err)
...
func (f ReindexList) WithContext(v context.Context) func(*ReindexListRequest) {
	return func(r *ReindexListRequest) {
		r.ctx = v
	}
}

// WithDetailed - return detailed reindex information (default: false).
...
func (f ReindexList) WithPretty() func(*ReindexListRequest) {
	return func(r *ReindexListRequest) {
		r.Pretty = true
	}
}
...
func (f ReindexList) WithHuman() func(*ReindexListRequest) {
	return func(r *ReindexListRequest) {
		r.Human = true
	}
}
