...
func newReindexCancelFunc(t Transport) ReindexCancel {
	return func(task_id string, o ...func(*ReindexCancelRequest)) (*Response, error) {
		var r = ReindexCancelRequest{TaskID: task_id}
		for _, f := range o {
			f(&r)
		}
...
func (r ReindexCancelRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		ctx = instrument.Start(providedCtx, "reindex_cancel")
	...
	path.Grow(7 + 1 + len("_reindex") + 1 + len(r.TaskID) + 1 + len("_cancel"))
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.BeforeRequest(req, "reindex_cancel")
	}
	res, err := transport.Perform(req)
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "reindex_cancel")
	}
	if err != nil {
		if instrument, ok := r.Instrument.(Instrumentation); ok {
			instrument.RecordError(ctx, err)
...
func (f ReindexCancel) WithContext(v context.Context) func(*ReindexCancelRequest) {
	return func(r *ReindexCancelRequest) {
		r.ctx = v
	}
}

// WithWaitForCompletion - should the request block until the cancellation of the reindex operation is completed. defaults to true.
