...
func (r IndicesPutDataStreamMappingsRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "indices.put_data_stream_mappings", r.Body); reader != nil {
			req.Body = reader
		}
	}
...
// WithDryRun - whether this request should only be a dry run rather than actually applying mappings.
