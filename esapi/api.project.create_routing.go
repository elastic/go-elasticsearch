...
func newProjectCreateRoutingFunc(t Transport) ProjectCreateRouting {
	return func(name string, body io.Reader, o ...func(*ProjectCreateRoutingRequest)) (*Response, error) {
		var r = ProjectCreateRoutingRequest{Name: name, Body: body}
		for _, f := range o {
			f(&r)
		}
...
// ProjectCreateRouting create or update named project routing expression
//
...
// ProjectCreateRoutingRequest configures the Project Create Routing API request.
...
func (r ProjectCreateRoutingRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.BeforeRequest(req, "project.create_routing")
		if reader := instrument.RecordRequestBody(ctx, "project.create_routing", r.Body); reader != nil {
			req.Body = reader
		}
	}
	res, err := transport.Perform(req)
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "project.create_routing")
	}
	if err != nil {
		if instrument, ok := r.Instrument.(Instrumentation); ok {
			instrument.RecordError(ctx, err)
...
func (f ProjectCreateRouting) WithContext(v context.Context) func(*ProjectCreateRoutingRequest) {
	return func(r *ProjectCreateRoutingRequest) {
		r.ctx = v
	}
}
