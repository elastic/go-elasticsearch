...
func newInferencePutGroqFunc(t Transport) InferencePutGroq {
	return func(body io.Reader, groq_inference_id string, task_type string, o ...func(*InferencePutGroqRequest)) (*Response, error) {
		var r = InferencePutGroqRequest{Body: body, GroqInferenceID: groq_inference_id, TaskType: task_type}
		for _, f := range o {
			f(&r)
		}
...
// InferencePutGroq create a Groq inference endpoint
//
// See full documentation at https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-inference-put-groq.
...
// InferencePutGroqRequest configures the Inference Put Groq API request.
...
func (r InferencePutGroqRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	path.Grow(7 + 1 + len("_inference") + 1 + len(r.TaskType) + 1 + len(r.GroqInferenceID))
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.RecordPathPart(ctx, "groq_inference_id", r.GroqInferenceID)
	}
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "inference.put_groq", r.Body); reader != nil {
			req.Body = reader
		}
	}
...
func (f InferencePutGroq) WithContext(v context.Context) func(*InferencePutGroqRequest) {
	return func(r *InferencePutGroqRequest) {
		r.ctx = v
	}
}
