...
func newInferencePutNvidiaFunc(t Transport) InferencePutNvidia {
	return func(body io.Reader, nvidia_inference_id string, task_type string, o ...func(*InferencePutNvidiaRequest)) (*Response, error) {
		var r = InferencePutNvidiaRequest{Body: body, NvidiaInferenceID: nvidia_inference_id, TaskType: task_type}
		for _, f := range o {
			f(&r)
		}
...
// InferencePutNvidiaRequest configures the Inference Put Nvidia API request.
...
func (r InferencePutNvidiaRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	path.Grow(7 + 1 + len("_inference") + 1 + len(r.TaskType) + 1 + len(r.NvidiaInferenceID))
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.RecordPathPart(ctx, "nvidia_inference_id", r.NvidiaInferenceID)
	}
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "inference.put_nvidia", r.Body); reader != nil {
			req.Body = reader
		}
	}
...
func (f InferencePutNvidia) WithContext(v context.Context) func(*InferencePutNvidiaRequest) {
	return func(r *InferencePutNvidiaRequest) {
		r.ctx = v
	}
}
