...
func newInferencePutLlamaFunc(t Transport) InferencePutLlama {
	return func(body io.Reader, llama_inference_id string, task_type string, o ...func(*InferencePutLlamaRequest)) (*Response, error) {
		var r = InferencePutLlamaRequest{Body: body, LlamaInferenceID: llama_inference_id, TaskType: task_type}
		for _, f := range o {
			f(&r)
		}
...
// InferencePutLlama create a Llama inference endpoint
//
// See full documentation at https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-inference-put-llama.
...
// InferencePutLlamaRequest configures the Inference Put Llama API request.
...
func (r InferencePutLlamaRequest) Do(providedCtx context.Context, transport Transport) (*Response, error) {
	...
	path.Grow(7 + 1 + len("_inference") + 1 + len(r.TaskType) + 1 + len(r.LlamaInferenceID))
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		instrument.RecordPathPart(ctx, "llama_inference_id", r.LlamaInferenceID)
	}
	...
	if instrument, ok := r.Instrument.(Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "inference.put_llama", r.Body); reader != nil {
			req.Body = reader
		}
	}
