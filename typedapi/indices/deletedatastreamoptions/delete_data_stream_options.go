...
// NewDeleteDataStreamOptions type alias for index.
type NewDeleteDataStreamOptions func(name string) *DeleteDataStreamOptions

// NewDeleteDataStreamOptionsFunc returns a new instance of DeleteDataStreamOptions with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewDeleteDataStreamOptionsFunc(tp elastictransport.Interface) NewDeleteDataStreamOptions {
	return func(name string) *DeleteDataStreamOptions {
		n := New(tp)
...
// Removes the data stream options from a data stream.
//
// https://www.elastic.co/docs/api/doc/elasticsearch/v8/operation/operation-indices-delete-data-stream-options
...
func (r DeleteDataStreamOptions) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "indices.delete_data_stream_options", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the DeleteDataStreamOptions query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a deletedatastreamoptions.Response
...
func (r DeleteDataStreamOptions) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the DeleteDataStreamOptions query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Header set a key, value pair in the DeleteDataStreamOptions headers map.
