...
// during regular segment merges. But force merge can cause very large (greater
// than 5 GB) segments to be produced, which are not eligible for regular
// merges. So the number of soft-deleted documents can then grow rapidly,
// resulting in higher disk usage and worse search performance. If you regularly
// force merge an index receiving writes, this can also make snapshots more
// expensive, since the new documents can't be backed up incrementally.
//
// # Blocks during a force merge
//
// Calls to this API block until the merge is complete (unless request contains
// `wait_for_completion=false`). If the client connection is lost before
// completion then the force merge process will continue in the background. Any
// new requests to force merge the same indices will also block until the
...
//	POST /.ds-my-data-stream-2099.03.07-000001/_forcemerge?max_num_segments=1
//
// https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-indices-forcemerge
