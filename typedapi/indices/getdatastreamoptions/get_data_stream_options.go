...
// NewGetDataStreamOptionsFunc returns a new instance of GetDataStreamOptions with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewGetDataStreamOptionsFunc(tp elastictransport.Interface) NewGetDataStreamOptions {
	return func(name string) *GetDataStreamOptions {
		n := New(tp)
...
// Get the data stream options configuration of one or more data streams.
//
// https://www.elastic.co/docs/api/doc/elasticsearch/v8/operation/operation-indices-get-data-stream-options
...
func (r GetDataStreamOptions) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "indices.get_data_stream_options", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the GetDataStreamOptions query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
func (r GetDataStreamOptions) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the GetDataStreamOptions query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// comma-separated values, such as `open,hidden`. Valid values are: `all`,
