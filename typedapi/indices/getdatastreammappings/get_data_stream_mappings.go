...
// NewGetDataStreamMappingsFunc returns a new instance of GetDataStreamMappings with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewGetDataStreamMappingsFunc(tp elastictransport.Interface) NewGetDataStreamMappings {
	return func(name string) *GetDataStreamMappings {
		n := New(tp)
...
// Get mapping information for one or more data streams.
//
// https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation/operation-indices-get-data-stream-mappings
...
func (r GetDataStreamMappings) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "indices.get_data_stream_mappings", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the GetDataStreamMappings query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
func (r GetDataStreamMappings) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the GetDataStreamMappings query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
