...
// NewGetDataStreamSettings type alias for index.
type NewGetDataStreamSettings func(name string) *GetDataStreamSettings

// NewGetDataStreamSettingsFunc returns a new instance of GetDataStreamSettings with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewGetDataStreamSettingsFunc(tp elastictransport.Interface) NewGetDataStreamSettings {
	return func(name string) *GetDataStreamSettings {
		n := New(tp)
...
// Get setting information for one or more data streams.
//
// [Elasticsearch] https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-indices-get-data-stream-settings
//
...
func (r GetDataStreamSettings) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "indices.get_data_stream_settings", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the GetDataStreamSettings query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a getdatastreamsettings.Response
...
func (r GetDataStreamSettings) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the GetDataStreamSettings query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Header set a key, value pair in the GetDataStreamSettings headers map.
