...
// ID, use the `POST /<target>/_doc/` request format and omit this parameter.
// API Name: id
...
// doesn't exist and doesn't match a data stream template, this request creates
// the index. You can check for existing targets with the resolve index API.
// API Name: index
...
// if absent). If a document with the specified `_id` already exists, the
// indexing operation will fail. The behavior is the same as using the
// `<index>/_create` endpoint. If a document ID is specified, this paramater
// defaults to `index`. Otherwise, it defaults to `create`. If the request
...
// has a default ingest pipeline specified, then setting the value to `_none`
// disables the default ingest pipeline for this request. If a final pipeline is
...
// API name: _slice
func (r *Index) RouteSlice_(routeslice_ string) *Index {
...
// Version An explicit version number for concurrency control. It must be a non-negative
// long number.
// API name: version
...
// operation. You can set it to `all` or any positive integer up to the total
// number of shards in the index (`number_of_replicas+1`). The default value of
// `1` means it waits for each primary shard to be active.
