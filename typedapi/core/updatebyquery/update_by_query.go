...
// visible to search after the request completes. This is different than the
// update API's `refresh` parameter, which causes just the shard that received
// the request to be refreshed.
// API name: refresh
...
// RequestCache If `true`, the request cache is used for this request. It defaults to the
// index-level setting.
...
// SearchTimeout An explicit timeout for each search request. By default, there is no timeout.
...
// returns a task ID that you can use to cancel or get the status of the task.
