...
// setting `conflicts` to `proceed`. Note that if you opt to count version
// conflicts the operation could attempt to delete more documents from the
// source than `max_docs` until it has successfully deleted `max_docs
...
// This pads each batch with a wait time to throttle the rate. Set
// `requests_per_second` to `-1` to disable throttling.
//
...
// `requests_per_second` and the time spent writing. By default the batch size
// is `1000`, so if `requests_per_second` is set to `500`:
//
...
// set. This is "bursty" instead of "smooth".
//
// # Slicing
//
...
// there are multiple source data streams or indices, it will choose the number
// of slices based on the index or backing index with the smallest number of
// shards. Adding slices to the delete by query operation creates sub-requests
// which means it has some quirks:
//
...
//     portion of the documents. All documents will be addressed, but some
//     slices may be larger than others. Expect larger slices to have a more
//     even distribution.
...
//     the number of shards in the index or backing index. If that number is
//     large (for example, 500), choose a lower number as too many `slices`
//     hurts performance. Setting `slices` higher than the number of shards
//     generally does not improve efficiency and adds overhead.
...
// Analyzer Analyzer to use for the query string. This parameter can be used only when
...
// after the request completes. This is different than the delete API's
...
// RequestCache If `true`, the request cache is used for this request. Defaults to the
// index-level setting.
...
// API name: _slice
func (r *DeleteByQuery) RouteSlice_(routeslice_ string) *DeleteByQuery {
...
// SearchTimeout The explicit timeout for each search request. It defaults to no timeout.
...
// TerminateAfter The maximum number of documents to collect for each shard. If a query reaches
...
// shards in the index (`number_of_replicas+1`). The `timeout` value controls
// how long each write request waits for unavailable shards to become available.
...
// WaitForCompletion If `true`, the request blocks until the operation is complete. If `false`,
// Elasticsearch performs some preflight checks, launches the request, and
// returns a task you can use to cancel or get the status of the task.
