...
// NewCancelReindex type alias for index.
type NewCancelReindex func(taskid string) *CancelReindex

// NewCancelReindexFunc returns a new instance of CancelReindex with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewCancelReindexFunc(tp elastictransport.Interface) NewCancelReindex {
	return func(taskid string) *CancelReindex {
		n := New(tp)
...
func (r CancelReindex) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		if r.spanStarted == false {
			ctx = instrument.Start(providedCtx, "cancel_reindex")
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.BeforeRequest(req, "cancel_reindex")
		if reader := instrument.RecordRequestBody(ctx, "cancel_reindex", r.raw); reader != nil {
			req.Body = reader
		}
	}
	res, err := r.transport.Perform(req)
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "cancel_reindex")
	}
	if err != nil {
		localErr := fmt.Errorf("an error happened during the CancelReindex query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a cancelreindex.Response
...
func (r CancelReindex) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the CancelReindex query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Header set a key, value pair in the CancelReindex headers map.
...
// TaskId The ID of the reindex task to cancel.
// API Name: taskid
...
// WaitForCompletion If `true` (the default), the request blocks until the cancellation is
// complete and returns the final task state. If `false`, the request returns
...
// API name: human
func (r *CancelReindex) Human(human bool) *CancelReindex {
...
// API name: pretty
func (r *CancelReindex) Pretty(pretty bool) *CancelReindex {
