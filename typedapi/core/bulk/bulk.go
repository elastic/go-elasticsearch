...
// Pipeline The pipeline identifier to use to preprocess incoming documents. If the index
...
// visible to search. If `wait_for`, wait for a refresh to make this operation
// visible to search. If `false`, do nothing with refreshes. Valid values:
// `true`, `false`, `wait_for`.
// API name: refresh
...
// API name: _slice
func (r *Bulk) RouteSlice_(routeslice_ string) *Bulk {
...
// is `1m` (one minute), which guarantees Elasticsearch waits for at least the
// timeout before failing. The actual wait time could be longer, particularly
...
// shards in the index (`number_of_replicas+1`). The default is `1`, which waits
// for each primary shard to be active.
