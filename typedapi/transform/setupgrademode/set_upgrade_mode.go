...
// transform info API.
//
// https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-transform-set-upgrade-mode
...
func (r SetUpgradeMode) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "transform.set_upgrade_mode", r.raw); reader != nil {
			req.Body = reader
		}
	}
...
// Enabled When `true`, it enables `upgrade_mode` which temporarily halts all transform
// tasks and prohibits new transform tasks from starting.
// API name: enabled
