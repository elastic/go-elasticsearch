...
// does not have a header row, columns are named "column1", "column2",
// "column3", for example.
...
// DocumentsToSample The number of documents to include in the structural analysis. The minimum
// value is 2.
...
// an impact when a whole message Grok pattern such as `%{CATALINALOG}` matches
// the input. If the structure finder identifies a common structure but has no
// idea of the meaning then generic field names such as `path`, `ipaddress`,
// `field1`, and `field2` are used in the `grok_pattern` output. The intention
// in that situation is that a user who knows the meanings will rename the
...
// Format The high level structure of the text. By default, the API chooses the format.
...
// is used to extract fields from every message in the text. The name of the
// timestamp field in the Grok pattern must match what is specified in the
// `timestamp_field` parameter. If that parameter is not specified, the name of
// the timestamp field in the Grok pattern must match "timestamp". If
...
// values in each row if they contain newlines or the delimiter character. Only
// a single character is supported. If this parameter is not specified, the
// default value is a double quote (`"`). If your delimited text format does not
// use quoting, a workaround is to set this argument to a character that does
// not appear anywhere in the sample.
// API name: quote
...
// TimestampFormat The Java time format of the timestamp field in the text. Only a subset of
...
// a period (`.`), comma (`,`), or colon (`:`). Spacing and punctuation is also
// permitted with the exception a question mark (`?`), newline, and carriage
// return, together with literal text enclosed in single quotes. For example,
// `MM/dd HH.mm.ss,SSSSSS 'in' yyyy` is a valid override format.
//
