...
// `hugging_face` service. Supported tasks include: `text_embedding`,
...
// intend to use.
//
// For Elastic's `text_embedding` task: The selected model must support the
// `Sentence Embeddings` task. On the new endpoint creation page, select the
// `Sentence Embeddings` task under the `Advanced Configuration` section. After
// the endpoint has initialized, copy the generated endpoint URL. Recommended
// models for `text_embedding` task:
//
//   - `all-MiniLM-L6-v2`
//   - `all-MiniLM-L12-v2`
//   - `all-mpnet-base-v2`
...
//
// For Elastic's `chat_completion` and `completion` tasks: The selected model
// must support the `Text Generation` task and expose OpenAI API. HuggingFace
// supports both serverless and dedicated endpoints for `Text Generation`. When
// creating dedicated endpoint select the `Text Generation` task. After the
// endpoint is initialized (for dedicated) or ready (for serverless), ensure it
// supports the OpenAI API and includes `/v1/chat/completions` part in URL.
// Then, copy the full endpoint URL for use. Recommended models for
// `chat_completion` and `completion` tasks:
//
//   - `Mistral-7B-Instruct-v0.2`
//   - `QwQ-32B`
//   - `Phi-3-mini-128k-instruct`
//
// For Elastic's `rerank` task: The selected model must support the
// `sentence-ranking` task and expose OpenAI API. HuggingFace supports only
// dedicated (not serverless) endpoints for `Rerank` so far. After the endpoint
// is initialized, copy the full endpoint URL for use. Tested models for
// `rerank` task:
//
//   - `bge-reranker-base`
...
//   - `jina-reranker-v1-turbo-en-GGUF`
//
// [Elasticsearch] https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-inference-put-hugging-face
//
