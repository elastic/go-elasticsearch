...
// NewPutNvidia type alias for index.
type NewPutNvidia func(tasktype, nvidiainferenceid string) *PutNvidia

// NewPutNvidiaFunc returns a new instance of PutNvidia with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewPutNvidiaFunc(tp elastictransport.Interface) NewPutNvidia {
	return func(tasktype, nvidiainferenceid string) *PutNvidia {
		n := New(tp)

		n._tasktype(tasktype)

		n._nvidiainferenceid(nvidiainferenceid)
...
// service.
//
// [Elasticsearch] https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-inference-put-nvidia
//
...
func (r *PutNvidia) Raw(raw io.Reader) *PutNvidia {
	r.raw = raw
...
func (r *PutNvidia) Request(req *Request) *PutNvidia {
	r.req = req
...
func (r *PutNvidia) HttpRequest(ctx context.Context) (*http.Request, error) {
	...
	if r.raw == nil && r.req != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not serialise request for PutNvidia: %w", err)
		}
	...
	switch {
	case r.paramSet == tasktypeMask|nvidiainferenceidMask:
		path.WriteString("/")
		...
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordPathPart(ctx, "nvidiainferenceid", r.nvidiainferenceid)
		}
		path.WriteString(r.nvidiainferenceid)
...
func (r PutNvidia) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.BeforeRequest(req, "inference.put_nvidia")
		if reader := instrument.RecordRequestBody(ctx, "inference.put_nvidia", r.raw); reader != nil {
			req.Body = reader
		}
	}
	res, err := r.transport.Perform(req)
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "inference.put_nvidia")
	}
	if err != nil {
		localErr := fmt.Errorf("an error happened during the PutNvidia query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a putnvidia.Response
...
// Header set a key, value pair in the PutNvidia headers map.
func (r *PutNvidia) Header(key, value string) *PutNvidia {
...
// NvidiaInferenceId The unique identifier of the inference endpoint.
// API Name: nvidiainferenceid
func (r *PutNvidia) _nvidiainferenceid(nvidiainferenceid string) *PutNvidia {
	r.paramSet |= nvidiainferenceidMask
	r.nvidiainferenceid = nvidiainferenceid
...
// created.
// API name: timeout
func (r *PutNvidia) Timeout(duration string) *PutNvidia {
...
func (r *PutNvidia) ErrorTrace(errortrace bool) *PutNvidia {
...
func (r *PutNvidia) ServiceSettings(servicesettings types.NvidiaServiceSettingsVariant) *PutNvidia {
	...
	r.req.ServiceSettings = *servicesettings.NvidiaServiceSettingsCaster()
...
func (r *PutNvidia) TaskSettings(tasksettings types.NvidiaTaskSettingsVariant) *PutNvidia {
	...
	r.req.TaskSettings = tasksettings.NvidiaTaskSettingsCaster()
