...
// NewEmbedding type alias for index.
type NewEmbedding func(inferenceid string) *Embedding

// NewEmbeddingFunc returns a new instance of Embedding with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewEmbeddingFunc(tp elastictransport.Interface) NewEmbedding {
	return func(inferenceid string) *Embedding {
		n := New(tp)
...
func (r *Embedding) Raw(raw io.Reader) *Embedding {
	r.raw = raw
...
func (r *Embedding) Request(req *Request) *Embedding {
	r.req = req
...
func (r *Embedding) HttpRequest(ctx context.Context) (*http.Request, error) {
	...
	if r.raw == nil && r.req != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not serialise request for Embedding: %w", err)
		}
...
func (r Embedding) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "inference.embedding", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the Embedding query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a embedding.Response
...
// Header set a key, value pair in the Embedding headers map.
func (r *Embedding) Header(key, value string) *Embedding {
...
// API name: timeout
func (r *Embedding) Timeout(duration string) *Embedding {
...
func (r *Embedding) ErrorTrace(errortrace bool) *Embedding {
...
func (r *Embedding) Input(embeddinginput types.EmbeddingInputVariant) *Embedding {
	...
	r.req.Input = *embeddinginput.EmbeddingInputCaster()
