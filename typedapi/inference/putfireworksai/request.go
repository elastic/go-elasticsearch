...
// Request holds the request body struct for the package putfireworksai
//
// https://github.com/elastic/elasticsearch-specification/blob/eb2e22fb2ac404e676d19bcc7bb089647f029026/specification/inference/put_fireworksai/PutFireworksAiRequest.ts#L31-L91
type Request struct {
	...
	// the `fireworksai` service.
	ServiceSettings types.FireworksAIServiceSettings `json:"service_settings"`
	// TaskSettings Settings to configure the inference task. Applies only to the `completion` or
	// `chat_completion` task types. Not applicable to the `text_embedding` task
	// type. These settings are specific to the task type you specified.
...
func (r *Request) FromJSON(data string) (*Request, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not deserialise json into Putfireworksai request: %w", err)
	}
