...
// NewPutFireworksai type alias for index.
type NewPutFireworksai func(tasktype, fireworksaiinferenceid string) *PutFireworksai

// NewPutFireworksaiFunc returns a new instance of PutFireworksai with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewPutFireworksaiFunc(tp elastictransport.Interface) NewPutFireworksai {
	return func(tasktype, fireworksaiinferenceid string) *PutFireworksai {
		n := New(tp)

		n._tasktype(tasktype)

		n._fireworksaiinferenceid(fireworksaiinferenceid)
...
// `fireworksai` service.
//
// [Elasticsearch] https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-inference-put-fireworksai
//
// [Serverless] https://www.elastic.co/docs/api/doc/elasticsearch-serverless/operation/operation-inference-put-fireworksai
func New(tp elastictransport.Interface) *PutFireworksai {
	r := &PutFireworksai{
		transport: tp,
...
func (r *PutFireworksai) Raw(raw io.Reader) *PutFireworksai {
	r.raw = raw
...
func (r *PutFireworksai) HttpRequest(ctx context.Context) (*http.Request, error) {
	...
	if r.raw == nil && r.req != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not serialise request for PutFireworksai: %w", err)
		}
	...
	switch {
	case r.paramSet == tasktypeMask|fireworksaiinferenceidMask:
		path.WriteString("/")
		...
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordPathPart(ctx, "fireworksaiinferenceid", r.fireworksaiinferenceid)
		}
		path.WriteString(r.fireworksaiinferenceid)
...
func (r PutFireworksai) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		if r.spanStarted == false {
			ctx = instrument.Start(providedCtx, "inference.put_fireworksai")
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.BeforeRequest(req, "inference.put_fireworksai")
		if reader := instrument.RecordRequestBody(ctx, "inference.put_fireworksai", r.raw); reader != nil {
			req.Body = reader
		}
	}
	res, err := r.transport.Perform(req)
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "inference.put_fireworksai")
	}
	if err != nil {
		localErr := fmt.Errorf("an error happened during the PutFireworksai query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a putfireworksai.Response
...
// Header set a key, value pair in the PutFireworksai headers map.
...
// FireworksaiInferenceId The unique identifier of the inference endpoint.
// API Name: fireworksaiinferenceid
func (r *PutFireworksai) _fireworksaiinferenceid(fireworksaiinferenceid string) *PutFireworksai {
	r.paramSet |= fireworksaiinferenceidMask
	r.fireworksaiinferenceid = fireworksaiinferenceid
...
// API name: human
func (r *PutFireworksai) Human(human bool) *PutFireworksai {
...
// API name: pretty
func (r *PutFireworksai) Pretty(pretty bool) *PutFireworksai {
...
func (r *PutFireworksai) ServiceSettings(servicesettings types.FireworksAIServiceSettingsVariant) *PutFireworksai {
	...
	r.req.ServiceSettings = *servicesettings.FireworksAIServiceSettingsCaster()

	return r
}

// Settings to configure the inference task. Applies only to the `completion` or
...
func (r *PutFireworksai) TaskSettings(tasksettings types.FireworksAITaskSettingsVariant) *PutFireworksai {
	...
	r.req.TaskSettings = tasksettings.FireworksAITaskSettingsCaster()
