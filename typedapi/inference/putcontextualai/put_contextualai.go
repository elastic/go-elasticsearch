...
// NewPutContextualaiFunc returns a new instance of PutContextualai with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewPutContextualaiFunc(tp elastictransport.Interface) NewPutContextualai {
	return func(tasktype, contextualaiinferenceid string) *PutContextualai {
		n := New(tp)

		n._tasktype(tasktype)

		n._contextualaiinferenceid(contextualaiinferenceid)
...
func (r *PutContextualai) HttpRequest(ctx context.Context) (*http.Request, error) {
	...
	if r.raw == nil && r.req != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not serialise request for PutContextualai: %w", err)
		}
	...
	switch {
	case r.paramSet == tasktypeMask|contextualaiinferenceidMask:
		path.WriteString("/")
		...
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordPathPart(ctx, "contextualaiinferenceid", r.contextualaiinferenceid)
		}
...
func (r PutContextualai) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "inference.put_contextualai", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the PutContextualai query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// ContextualaiInferenceId The unique identifier of the inference endpoint.
// API Name: contextualaiinferenceid
func (r *PutContextualai) _contextualaiinferenceid(contextualaiinferenceid string) *PutContextualai {
	...
	r.contextualaiinferenceid = contextualaiinferenceid
...
func (r *PutContextualai) ServiceSettings(servicesettings types.ContextualAIServiceSettingsVariant) *PutContextualai {
	...
	r.req.ServiceSettings = *servicesettings.ContextualAIServiceSettingsCaster()
...
func (r *PutContextualai) TaskSettings(tasksettings types.ContextualAITaskSettingsVariant) *PutContextualai {
	...
	r.req.TaskSettings = tasksettings.ContextualAITaskSettingsCaster()
