...
// NewFlamegraphFunc returns a new instance of Flamegraph with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewFlamegraphFunc(tp elastictransport.Interface) NewFlamegraph {
	return func() *Flamegraph {
		n := New(tp)
...
func (r *Flamegraph) Raw(raw io.Reader) *Flamegraph {
	r.raw = raw
...
func (r *Flamegraph) Request(req any) *Flamegraph {
	r.req = req
...
func (r *Flamegraph) Conditions(conditions any) *Flamegraph {
	r.req = conditions
...
func (r *Flamegraph) HttpRequest(ctx context.Context) (*http.Request, error) {
	...
	if r.raw == nil && r.req != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not serialise request for Flamegraph: %w", err)
		}
...
func (r Flamegraph) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "profiling.flamegraph", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the Flamegraph query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
func (r *Flamegraph) ErrorTrace(errortrace bool) *Flamegraph {
...
// only by machines.
// API name: human
func (r *Flamegraph) Human(human bool) *Flamegraph {
...
// API name: pretty
func (r *Flamegraph) Pretty(pretty bool) *Flamegraph {
