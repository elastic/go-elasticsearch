...
// NewTags type alias for index.
type NewTags func() *Tags

// NewTagsFunc returns a new instance of Tags with the provided transport.
...
func New(tp elastictransport.Interface) *Tags {
	r := &Tags{
		transport: tp,
...
func (r Tags) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "project.tags", r.raw); reader != nil {
			req.Body = reader
		}
	}
	res, err := r.transport.Perform(req)
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "project.tags")
	}
	if err != nil {
		localErr := fmt.Errorf("an error happened during the Tags query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a tags.Response
...
func (r *Tags) ErrorTrace(errortrace bool) *Tags {
...
func (r *Tags) FilterPath(filterpaths ...string) *Tags {
	tmp := []string{}
