...
// NewCreateManyRouting type alias for index.
type NewCreateManyRouting func() *CreateManyRouting

// NewCreateManyRoutingFunc returns a new instance of CreateManyRouting with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewCreateManyRoutingFunc(tp elastictransport.Interface) NewCreateManyRouting {
	return func() *CreateManyRouting {
		n := New(tp)
...
func (r *CreateManyRouting) HttpRequest(ctx context.Context) (*http.Request, error) {
	...
	if r.raw == nil && r.req != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("could not serialise request for CreateManyRouting: %w", err)
		}
...
func (r CreateManyRouting) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "project.create_many_routing", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the CreateManyRouting query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a createmanyrouting.Response
...
// Header set a key, value pair in the CreateManyRouting headers map.
