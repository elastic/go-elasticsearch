...
// NewGetManyRouting type alias for index.
type NewGetManyRouting func() *GetManyRouting

// NewGetManyRoutingFunc returns a new instance of GetManyRouting with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewGetManyRoutingFunc(tp elastictransport.Interface) NewGetManyRouting {
	return func() *GetManyRouting {
		n := New(tp)
...
func (r GetManyRouting) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "project.get_many_routing", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the GetManyRouting query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a getmanyrouting.Response
...
func (r GetManyRouting) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the GetManyRouting query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Header set a key, value pair in the GetManyRouting headers map.
...
// only by machines.
// API name: human
func (r *GetManyRouting) Human(human bool) *GetManyRouting {
...
// API name: pretty
func (r *GetManyRouting) Pretty(pretty bool) *GetManyRouting {
