...
// NewSecretDelete type alias for index.
type NewSecretDelete func(id string) *SecretDelete

// NewSecretDeleteFunc returns a new instance of SecretDelete with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewSecretDeleteFunc(tp elastictransport.Interface) NewSecretDelete {
	return func(id string) *SecretDelete {
		n := New(tp)
...
// Deletes a connector secret.
func New(tp elastictransport.Interface) *SecretDelete {
	r := &SecretDelete{
		transport: tp,
...
func (r SecretDelete) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "connector.secret_delete", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the SecretDelete query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a secretdelete.Response
...
func (r SecretDelete) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the SecretDelete query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Header set a key, value pair in the SecretDelete headers map.
...
// API Name: id
func (r *SecretDelete) _id(id string) *SecretDelete {
	r.paramSet |= idMask
	r.id = id
...
// only by machines.
// API name: human
func (r *SecretDelete) Human(human bool) *SecretDelete {
...
// API name: pretty
func (r *SecretDelete) Pretty(pretty bool) *SecretDelete {
