...
import (
	...
	"github.com/elastic/go-elasticsearch/v9/typedapi/types/enums/statslevel"
)
...
// Get searchable snapshot statistics.
//
// https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-searchable-snapshots-stats
...
func (r Stats) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "searchable_snapshots.stats", r.raw); reader != nil {
			req.Body = reader
		}
	}
