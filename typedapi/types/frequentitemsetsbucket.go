...
//
// https://github.com/elastic/elasticsearch-specification/blob/2f823ff6fcaa7f3f0f9b990dc90512d8901e5d64/specification/_types/aggregations/Aggregate.ts#L725-L728
...
func (s *FrequentItemSetsBucket) UnmarshalJSON(data []byte) error {
	...
	for {
		...
		case "key":
			if s.Key == nil {
				s.Key = make(map[string][]string, 0)
			}
		...
		case "support":
			...
			case string:
				...
				if err != nil {
					return fmt.Errorf("%s | %w", "Support", err)
				}
				f := Float64(value)
				s.Support = f
			case float64:
