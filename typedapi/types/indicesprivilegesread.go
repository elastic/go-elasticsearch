...
//
// https://github.com/elastic/elasticsearch-specification/blob/37285cbd3fd155f913b50d880b40ec45f9df64b3/specification/security/get_role/types.ts#L33-L48
type IndicesPrivilegesRead struct {
	...
	// ImplicitlyGranted Set to `true` on entries that were contributed by a registered
	// `ImplicitPrivilegesProvider` rather than explicitly stored on the role. Only
	// present when the get role API is called with `include_implicit=true`. The put
	// role API rejects this field, so clients must not echo it back on a
	...
	for {
		...
		case "implicitly_granted":
			...
			case string:
				value, err := strconv.ParseBool(v)
				if err != nil {
					return fmt.Errorf("%s | %w", "ImplicitlyGranted", err)
				}
				s.ImplicitlyGranted = &value
			case bool:
				s.ImplicitlyGranted = &v
			}
...
// NewIndicesPrivilegesRead returns a IndicesPrivilegesRead.
func NewIndicesPrivilegesRead() *IndicesPrivilegesRead {
	r := &IndicesPrivilegesRead{}
