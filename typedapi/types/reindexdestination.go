...
//
// https://github.com/elastic/elasticsearch-specification/blob/7fd0bd13eaf28bd179fc906f57da09e852eb818e/specification/_global/reindex/types.ts#L39-L76
type ReindexDestination struct {
	...
	// RouteSlice_ The slice identifier used to route the reindexed documents to a specific
	...
	RouteSlice_ *string `json:"_slice,omitempty"`
...
func (s *ReindexDestination) UnmarshalJSON(data []byte) error {
	...
	for {
		...
		case "_slice":
			var tmp json.RawMessage
			if err := dec.Decode(&tmp); err != nil {
				return fmt.Errorf("%s | %w", "RouteSlice_", err)
			}
			o := string(tmp[:])
			o, err = strconv.Unquote(o)
			if err != nil {
