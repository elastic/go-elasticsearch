...
//
// https://github.com/elastic/elasticsearch-specification/blob/6ee016a765be615b0205fc209d3d3c515044689d/specification/_global/search/_types/suggester.ts#L359-L413
type PhraseSuggester struct {
	...
	// GramSize Sets max size of the n-grams (shingles) in the field. If the field doesn’t
	// contain n-grams (shingles), this should be omitted or set to `1`. If the
	// field uses a shingle filter, the `gram_size` is set to the `max_shingle_size`
	// if not explicitly set.
	GramSize *int `json:"gram_size,omitempty"`
	// Highlight Sets up suggestion highlighting. If not provided, no highlighted field is
	// returned.
	...
	// form a correction. This method accepts a float value in the range `[0..1)` as
	// a fraction of the actual query terms or a number `>=1` as an absolute number
	// of query terms.
	...
	// Separator The separator that is used to separate terms in the bigram field. If not set,
