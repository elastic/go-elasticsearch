...
// IndexSettingsUnassignedNodeLeft type.
//
// https://github.com/elastic/elasticsearch-specification/blob/37285cbd3fd155f913b50d880b40ec45f9df64b3/specification/indices/_types/IndexSettings.ts#L358-L365
type IndexSettingsUnassignedNodeLeft struct {
	// DelayedTimeout The amount of time to wait for a node that has left before assuming its
...
func (s *IndexSettingsUnassignedNodeLeft) UnmarshalJSON(data []byte) error {
	...
	for {
		...
		case "delayed_timeout":
			if err := dec.Decode(&s.DelayedTimeout); err != nil {
				return fmt.Errorf("%s | %w", "DelayedTimeout", err)
			}
...
// NewIndexSettingsUnassignedNodeLeft returns a IndexSettingsUnassignedNodeLeft.
func NewIndexSettingsUnassignedNodeLeft() *IndexSettingsUnassignedNodeLeft {
	r := &IndexSettingsUnassignedNodeLeft{}
