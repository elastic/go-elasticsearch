...
//
// https://github.com/elastic/elasticsearch-specification/blob/2f823ff6fcaa7f3f0f9b990dc90512d8901e5d64/specification/_types/mapping/complex.ts#L26-L37
	...
	for {
		...
		case "depth_limit":
			...
			case string:
				...
				if err != nil {
					return fmt.Errorf("%s | %w", "DepthLimit", err)
				}
				s.DepthLimit = &value
			case float64:
				f := int(v)
		...
		case "split_queries_on_whitespace":
			...
			case string:
				value, err := strconv.ParseBool(v)
				if err != nil {
					return fmt.Errorf("%s | %w", "SplitQueriesOnWhitespace", err)
				}
