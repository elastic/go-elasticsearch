...
//
// https://github.com/elastic/elasticsearch-specification/blob/bc885996c471cc7c2c7d51cba22aab19867672ac/specification/inference/_types/CommonTypes.ts#L1409-L1465
type ElasticsearchServiceSettings struct {
	...
	// DeploymentId The deployment identifier for a trained model deployment. When
	// `deployment_id` is used the `model_id` is optional.
	...
	// MaxChunksPerDoc Only for the `rerank` task type. Limits the number of chunks per document
	// that are sent for inference when chunking is enabled. If not set, all chunks
	// generated for the document are processed.
	MaxChunksPerDoc *int `json:"max_chunks_per_doc,omitempty"`
	// ModelId The name of the model to use for the inference task. It can be the ID of a
	// built-in model (for example, `.multilingual-e5-small` for E5) or a text
	...
	// NumThreads The number of threads used by each model allocation during inference. This
	// setting generally increases the speed per inference request. The inference
	// process is a compute-bound process; `threads_per_allocations` must not exceed
	// the number of available allocated processors per node. The value must be a
	// power of 2. The maximum value is 32.
