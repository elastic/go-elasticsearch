...
// RangeBucket type.
//
// https://github.com/elastic/elasticsearch-specification/blob/2f823ff6fcaa7f3f0f9b990dc90512d8901e5d64/specification/_types/aggregations/Aggregate.ts#L600-L607
	...
	for {
		...
		case "from_as_string":
			...
			if err := dec.Decode(&tmp); err != nil {
				return fmt.Errorf("%s | %w", "FromAsString", err)
			}
			o := string(tmp[:])
		...
		case "to_as_string":
			var tmp json.RawMessage
			if err := dec.Decode(&tmp); err != nil {
				return fmt.Errorf("%s | %w", "ToAsString", err)
			}
			o := string(tmp[:])
			o, err = strconv.Unquote(o)
			if err != nil {
