...
// IpPrefixBucket type.
//
// https://github.com/elastic/elasticsearch-specification/blob/2f823ff6fcaa7f3f0f9b990dc90512d8901e5d64/specification/_types/aggregations/Aggregate.ts#L715-L720
...
func (s *IpPrefixBucket) UnmarshalJSON(data []byte) error {
	...
	for {
		...
		case "netmask":
			var tmp json.RawMessage
			if err := dec.Decode(&tmp); err != nil {
				return fmt.Errorf("%s | %w", "Netmask", err)
			}
			o := string(tmp[:])
			o, err = strconv.Unquote(o)
			if err != nil {
