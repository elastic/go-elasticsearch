...
//
// https://github.com/elastic/elasticsearch-specification/blob/bc885996c471cc7c2c7d51cba22aab19867672ac/specification/ml/_types/Analysis.ts#L79-L148
type AnalysisConfigRead struct {
	...
	// Detectors An array of detector configuration objects. Detector configuration objects
	// specify which data fields a job analyzes. They also specify which analytical
	// functions are used. You can specify multiple detectors for a job.
	...
	// from the model. Typically, set to `30d` or longer. If not set, model pruning
	// only occurs if the model memory status reaches the soft limit or the hard
	// limit. For jobs created in 8.1 and later, the default value is the greater of
	// `30d` or 20 times `bucket_span`.
