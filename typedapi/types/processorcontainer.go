...
//
// https://github.com/elastic/elasticsearch-specification/blob/bc885996c471cc7c2c7d51cba22aab19867672ac/specification/ingest/_types/Processors.ts#L28-L307
type ProcessorContainer struct {
	...
	// CommunityId Computes the Community ID for network flow data as defined in the Community
	...
	// DotExpander Expands a field with dots into an object field. This processor allows fields
	...
	// Drop Drops the document without raising any errors. This is useful to prevent the
	...
	// Fail Raises an exception. This is useful for when you expect a pipeline to fail
	...
	// Grok Extracts structured fields out of a single text field within a document. You
	...
	// HtmlStrip Removes HTML tags from the field. If the field is an array of strings, HTML
	...
	// Lowercase Converts a string to its lowercase equivalent. If the field is an array of
	...
	// RegisteredDomain Extracts the registered domain (also known as the effective top-level domain
	...
	// Remove Removes existing fields. If one field doesn’t exist, an exception will be
	// thrown.
	Remove *RemoveProcessor `json:"remove,omitempty"`
	// Rename Renames an existing field. If the field doesn’t exist or the new name is
	// already used, an exception will be thrown.
	Rename *RenameProcessor `json:"rename,omitempty"`
	// Reroute Routes a document to another target index or data stream. When setting the
	...
	// Script Runs an inline or stored script on incoming documents. The script runs in the
	// `ingest` context.
	Script *ScriptProcessor `json:"script,omitempty"`
	// Set Adds a field with the specified value. If the field already exists, its value
	...
	// Sort Sorts the elements of an array ascending or descending. Homogeneous arrays of
	...
	// Split Splits a field into an array using a separator character. Only works on
	// string fields.
	...
	// Trim Trims whitespace from a field. If the field is an array of strings, all
	// members of the array will be trimmed. This only works on leading and trailing
	// whitespace.
	Trim *TrimProcessor `json:"trim,omitempty"`
	// Uppercase Converts a string to its uppercase equivalent. If the field is an array of
	// strings, all members of the array will be converted.
	...
	// Urldecode URL-decodes a string. If the field is an array of strings, all members of the
