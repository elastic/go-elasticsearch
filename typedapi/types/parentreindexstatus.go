...
// ParentReindexStatus type.
//
// https://github.com/elastic/elasticsearch-specification/blob/37285cbd3fd155f913b50d880b40ec45f9df64b3/specification/_global/reindex_rethrottle/types.ts#L46-L48
...
func (s *ParentReindexStatus) UnmarshalJSON(data []byte) error {
	...
	for {
		...
		case "slices":
			if err := dec.Decode(&s.Slices); err != nil {
				return fmt.Errorf("%s | %w", "Slices", err)
			}
		...
		case "throttled_until":
			if err := dec.Decode(&s.ThrottledUntil); err != nil {
				return fmt.Errorf("%s | %w", "ThrottledUntil", err)
			}
...
// NewParentReindexStatus returns a ParentReindexStatus.
func NewParentReindexStatus() *ParentReindexStatus {
	r := &ParentReindexStatus{}
