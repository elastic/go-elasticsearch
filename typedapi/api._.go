...
type Core struct {
	...
	// the same semantics as the `op_type` parameter in the standard index API. A
	// `create` action fails if a document with the same ID already exists in the
	// target An `index` action adds or replaces a document as necessary.
	//
	// NOTE: Data streams support only the `create` action. To update or delete a
	...
	// newline character may be preceded by a carriage return (`\r`). When sending
	// NDJSON data to the `_bulk` endpoint, use a `Content-Type` header of
	// `application/json` or `application/x-ndjson`. Because this format uses
	// literal newline characters (`\n`) as delimiters, make sure that the JSON
	// actions and sources are not pretty printed.
	//
	...
	// operations are run, based on the last modification to existing documents. See
	// Optimistic concurrency control for more details.
	//
	// # Versioning
	//
	// Each bulk item can include the version value using the `version` field. It
	// automatically follows the behavior of the index or delete operation based on
	// the `_version` mapping. It also support the `version_type`.
	//
	// # Routing
	//
	// Each bulk item can include the routing value using the `routing` field. It
	// automatically follows the behavior of the index or delete operation based on
	// the `_routing` mapping.
	//
	...
	// refresh. Imagine a `_bulk?refresh=wait_for` request with three documents in
	// it that happen to be routed to different shards in an index with five shards.
	// The request will only wait for those three shards to refresh. The other two
	...
	// optional. When no query is provided, the API uses `match_all` to count all
	// the documents.
	//
	...
	// `/<target>/_create/<_id>` APIs Using `_create` guarantees that the document
	// is indexed only if it does not already exist. It returns a 409 response when
	// a document with a same ID already exists in the index. To update an existing
	// document, you must use the `/<target>/_doc/` API.
	//
	// If the Elasticsearch security features are enabled, you must have the
	// following index privileges for the target data stream, index, or index alias:
	//
	...
	//     must have the `auto_configure`, `create_index`, or `manage` index
	//     privilege.
	//
	// Automatic data stream creation requires a matching index template with data
	// stream enabled.
	...
	// setting. If it is `true`, any index can be created automatically. You can
	// modify this setting to explicitly allow or block automatic creation of
	// indices that match specified patterns or set it to `false` to turn off
	// automatic index creation entirely. Specify a comma-separated list of patterns
	...
	// of indices only. It does not affect the creation of data streams.
	//
	// # Routing
	//
	// By default, shard placement — or routing — is controlled by using a hash
	// of the document's ID value. For more explicit control, the value fed into the
	// hash function used by the router can be directly specified on a per-operation
	// basis using the `routing` parameter.
	//
	...
	// itself. This does come at the (very minimal) cost of an additional document
	// parsing pass. If the `_routing` mapping is defined and set to be required,
	...
	// still proceed with only one copy of the data. If `wait_for_active_shards` is
	// set on the request to `3` (and all three nodes are up), the indexing
	// operation will require 3 active shard copies before proceeding. This
	// requirement should be met because there are 3 active nodes in the cluster,
	// each one holding a copy of the shard. However, if you set
	// `wait_for_active_shards` to `all` (or to `4`, which is the same in this
	// situation), the indexing operation will not proceed as you do not have all 4
	// copies of each shard active in the index. The operation will timeout unless a
	// new node is brought up in the cluster to host the fourth copy of the shard.
	//
	...
	//     request format, you must have the `create`, `index`, or `write` index
	//     privilege.
	//   - To add a document using the `POST /<target>/_doc/` request format, you
	//     must have the `create_doc`, `create`, `index`, or `write` index
	//     privilege.
	//   - To automatically create a data stream or index with this API request, you
	//     must have the `auto_configure`, `create_index`, or `manage` index
	//     privilege.
	...
	// use the `_update` API with `detect_noop` set to `true`. The `detect_noop`
	// option isn't available on this API because it doesn’t fetch the old source
	...
	// `version_type` should be set to `external`. The value provided must be a
	// numeric, long value greater than or equal to 0, and less than around
	// `9.2e+18`.
	//
	...
	// host in the `reindex.remote.whitelist` setting. Automatic data stream
	// creation requires a matching index template with data stream enabled.
	//
	...
	// not a good idea to rely on this behavior. Instead, make sure that IDs are
	// unique by using a script.
	//
	...
	// to cancel or get the status of the task. Elasticsearch creates a record of
	...
	// operations. Requests are throttled by padding each batch with a wait time. To
	// turn off throttling, set `requests_per_second` to `-1`.
	//
	...
	// the `requests_per_second` and the time spent writing. By default the batch
	// size is `1000`, so if `requests_per_second` is set to `500`:
	//
	//	target_time = 1000 / 500 per second = 2 seconds
	//	wait_time = target_time - write_time = 2 seconds - .5 seconds = 1.5 seconds
	//
	...
	// parallelize by using sliced scroll to slice on `_id`. The `slices` parameter
	...
	//
	//   - You can see these requests in the tasks API. These sub-requests are
	//     "child" tasks of the task for the request with slices.
	//   - Fetching the status of the task for the request with `slices` only
	//     contains the status of completed slices.
	//   - These sub-requests are individually addressable for things like
	//     cancellation and rethrottling.
	//   - Rethrottling the request with `slices` will rethrottle the unfinished
	//     sub-request proportionally.
	//   - Canceling the request with `slices` will cancel each sub-request.
	//   - Due to the nature of `slices`, each sub-request won't get a perfectly
	//     even portion of the documents. All documents will be addressed, but some
	//     slices may be larger than others. Expect larger slices to have a more
	//     even distribution.
	//   - Parameters like `requests_per_second` and `max_docs` on a request with
	//     `slices` are distributed proportionally to each sub-request. Combine that
	...
	// number for most indices. If slicing manually or otherwise tuning automatic
	// slicing, use the following guidelines.
	//
	...
	// the document. Unlike `_update_by_query`, the script is allowed to modify the
	// document's metadata.
	//
	// Just as in `_update_by_query`, you can set `ctx.op` to change the operation
	// that is run on the destination. For example, set `ctx.op` to `noop` if your
	// script decides that the document doesn’t have to be indexed in the
	// destination. This "no operation" will be reported in the `noop` counter in
	// the response body. Set `ctx.op` to `delete` if your script decides that the
	// document must be deleted from the destination. The deletion will be reported
	// in the `deleted` counter in the response body. Setting `ctx.op` to anything
	// else will return an error, as will setting any other field in `ctx`.
	//
	...
	// not sending the version in an indexing request. It will cause the document to
	// be overwritten in the destination regardless of the version on the target or
	// the version type you use in the reindex API.
	//
	// # Reindex from remote
	//
	// Reindex supports reindexing from a remote Elasticsearch cluster. The `host`
	// parameter must contain a scheme, host, port, and optional path. The
	// `username` and `password` parameters are optional and when they are present
	// the reindex operation will connect to the remote Elasticsearch node using
	// basic authentication. Be sure to use HTTPS when using basic authentication or
	// the password will be sent in plain text. There are a range of settings
	// available to configure the behavior of the HTTPS connection.
	//
	...
	// explicitly allowed with the `reindex.remote.whitelist` setting. It can be set
	// to a comma delimited list of allowed remote host and port combinations.
	...
	// socket read timeout on the remote connection with the `socket_timeout` field
	// and the connection timeout with the `connect_timeout` field. Both default to
	// 30 seconds.
	//
	// # Configuring SSL parameters
	//
	// Reindex from remote supports configurable SSL settings. These must be
	// specified in the `elasticsearch.yml` file, with the exception of the secure
	// settings, which you add in the Elasticsearch keystore. It is not possible to
	// configure SSL in the body of the reindex request.
	...
	// search queries using the `q` query string parameter or the request body. If
	// both are specified, only the query parameter is used.
	//
	...
	//     `grid_agg` parameter determines the aggregation type. The aggregation
	//     uses the `<zoom>/<x>/<y>` tile as a bounding box.
	...
	//   - If the optional parameter `with_labels` is `true`, the internal search
	//     will include a dynamic runtime field that calls the `getLabelPosition`
	//     function of the geometry doc value. This enables the generation of new
	//     point features containing suggested geometry labels, so that, for
	//     example, multi-polygons will have only one label.
	//
	...
	//     or `geohex_grid`. The layer only contains features for cells with
	//     matching data.
	...
	// available through these cells. The final precision is computed by as follows:
	// `<zoom> + grid_precision`. For example, if `<zoom>` is 7 and `grid_precision`
	// is 8, then the `geotile_grid` aggregation will use a precision of 15. The
	// maximum final precision is 29. The `grid_precision` also determines the
	// number of cells for the grid as follows: `(2^grid_precision) x
	// (2^grid_precision)`. For example, a value of 8 divides the tile into a grid
	// of 256 x 256 cells. The `aggs` layer only contains features for cells with
	// matching data.
	//
	...
	// by the `geohex` aggregation. The following table maps the H3 resolution for
	// each precision. For example, if `<zoom>` is 3 and `grid_precision` is 3, the
	// precision is 6. At a precision of 6, hexagonal cells have an H3 resolution of
	// 2. If `<zoom>` is 3 and `grid_precision` is 4, the precision is 7. At a
	// precision of 7, hexagonal cells have an H3 resolution of 3.
	//
	// | Precision | Unique tile bins | H3 resolution | Unique hex bins | Ratio | |
	// --------- | ---------------- | ------------- | ----------------| ----- | | 1
	// | 4 | 0 | 122 | 30.5 | | 2 | 16 | 0 | 122 | 7.625 | | 3 | 64 | 1 | 842 |
	// 13.15625 | | 4 | 256 | 1 | 842 | 3.2890625 | | 5 | 1024 | 2 | 5882 |
	// 5.744140625 | | 6 | 4096 | 2 | 5882 | 1.436035156 | | 7 | 16384 | 3 | 41162 |
	// 2.512329102 | | 8 | 65536 | 3 | 41162 | 0.6280822754 | | 9 | 262144 | 4 |
	// 288122 | 1.099098206 | | 10 | 1048576 | 4 | 288122 | 0.2747745514 | | 11 |
	// 4194304 | 5 | 2016842 | 0.4808526039 | | 12 | 16777216 | 6 | 14117882 |
	// 0.8414913416 | | 13 | 67108864 | 6 | 14117882 | 0.2103728354 | | 14 |
	// 268435456 | 7 | 98825162 | 0.3681524172 | | 15 | 1073741824 | 8 | 691776122 |
	// 0.644266719 | | 16 | 4294967296 | 8 | 691776122 | 0.1610666797 | | 17 |
	// 17179869184 | 9 | 4842432842 | 0.2818666889 | | 18 | 68719476736 | 10 |
	// 33897029882 | 0.4932667053 | | 19 | 274877906944 | 11 | 237279209162 |
	// 0.8632167343 | | 20 | 1099511627776 | 11 | 237279209162 | 0.2158041836 | | 21
	// | 4398046511104 | 12 | 1660954464122 | 0.3776573213 | | 22 | 17592186044416 |
	// 13 | 11626681248842 | 0.6609003122 | | 23 | 70368744177664 | 13 |
	// 11626681248842 | 0.165225078 | | 24 | 281474976710656 | 14 | 81386768741882 |
	// 0.2891438866 | | 25 | 1125899906842620 | 15 | 569707381193162 | 0.5060018015
	// | | 26 | 4503599627370500 | 15 | 569707381193162 | 0.1265004504 | | 27 |
	// 18014398509482000 | 15 | 569707381193162 | 0.03162511259 | | 28 |
	// 72057594037927900 | 15 | 569707381193162 | 0.007906278149 | | 29 |
	// 288230376151712000 | 15 | 569707381193162 | 0.001976569537 |
	//
	...
	// setting `conflicts` to `proceed`. Note that if you opt to count version
	// conflicts, the operation could attempt to update more documents from the
	// source than `max_docs` until it has successfully updated `max_docs` documents
	...
	// operations, you can set `requests_per_second` to any positive decimal number.
	// This pads each batch with a wait time to throttle the rate. Set
	// `requests_per_second` to `-1` to turn off throttling.
	//
	// Throttling uses a wait time between batches so that the internal scroll
	// requests can be given a timeout that takes the request padding into account.
	// The padding time is the difference between the batch size divided by the
	...
	// and indices. This setting will use one slice per shard, up to a certain
	// limit. If there are multiple source data streams or indices, it will choose
	// the number of slices based on the index or backing index with the smallest
	// number of shards.
	//
	...
	//   - Parameters like `requests_per_second` and `max_docs` on a request with
	//     slices are distributed proportionally to each sub-request. Combine that
	//     with the point above about distribution being uneven and you should
	//     conclude that using `max_docs` with `slices` might not result in exactly
	//     `max_docs` documents being updated.
	//   - Each sub-request gets a slightly different snapshot of the source data
	//     stream or index though these are all taken at approximately the same
	//     time.
	//
	...
	//   - Query performance is most efficient when the number of slices is equal to
	//     the number of shards in the index or backing index. If that number is
	//     large (for example, 500), choose a lower number as too many slices hurts
	//     performance. Setting slices higher than the number of shards generally
	//     does not improve efficiency and adds overhead.
	...
	// update API, you can set `ctx.op` to change the operation that is performed.
	//
	// Set `ctx.op = "noop"` if your script decides that it doesn't have to make any
	// changes. The update by query operation skips updating the document and
	// increments the `noop` counter.
	//
	// Set `ctx.op = "delete"` if your script decides that the document should be
	// deleted. The update by query operation deletes the document and increments
	// the `deleted` counter.
	//
	// Update by query supports only `index`, `noop`, and `delete`. Setting `ctx.op`
	// to anything else is an error. Setting any other field in `ctx` is an error.
...
type Indices struct {
	...
	// using the `ignore_unavailable=true` parameter.
	//
	// By default, you must explicitly name the indices you are opening or closing.
	// To open or close indices with `_all`, `*`, or other wildcard expressions,
	// change the `action.destructive_requires_name` setting to `false`. This
	...
	// later component templates override earlier component templates. Any mappings,
	// settings, or aliases from the parent index template are merged in next.
	// Finally, any configuration on the index request itself is merged. Mapping
	// definitions are merged recursively, which means that later mapping components
	// can introduce new field mappings and update the mapping configuration. If a
	// field mapping is already contained in an earlier component, its definition
	// will be completely overwritten by the later one. This recursive merging
	// strategy applies not only to field mappings, but also root options like
	// `dynamic_templates` and `meta`. If an earlier component contains a
	...
	// cluster, including the local "querying" cluster, if included. If no index
	// expression is provided, the API will return information about all the remote
	...
	//     actively attempts to contact the remote clusters, unlike the
	//     `remote/info` endpoint.
	//   - Whether each remote cluster is configured with `skip_unavailable` as
	//     `true` or `false`.
	...
	//     cross-cluster search (including any authorization errors if you do not
	//     have permission to query the index).
	...
	// start with the alias `cluster*`. Each cluster returns information about
	// whether it has any indices, aliases or data streams that match `my-index-*`.
	//
	...
	// the index expression `dummy*` to those remote clusters. Thus, if an errors
	// occur, you may see a reference to that index expression even though you
	// didn't request it. If it causes a problem, you can instead include an index
	// expression like `*:*` to bypass the issue.
	//
	...
	//     remote1 cluster has no indices, aliases or data streams that match
	//     `logs*`. In that case, that cluster will return no results from that
	//     cluster if you include it in a cross-cluster search.
	//   - The index expression (combined with any query parameters you specify)
	//     will likely cause an exception to be thrown when you do the search. In
	//     these cases, the "error" field in the `_resolve/cluster` response will be
	//     present. (This is also where security/permission errors will be shown.)
	...
	// not. The remote cluster may be available, while the local cluster is not
	// currently connected to it.
	//
	// You can use the `_resolve/cluster` API to attempt to reconnect to remote
	// clusters. For example with `GET _resolve/cluster` or `GET
	...
	// `is_write_index` set to `true`. The API also `sets is_write_index` to `false`
	...
	// number. For example, if you roll over an alias with a current index of
	...
	// that points to an index named `<my-index-{now/d}-000001>`. If you create the
	...
	//     file system does not support hard-linking, then all segments are copied
	//     into the new index, which is a much more time consuming process. Also if
	//     using multiple data paths, shards on different data paths require a full
	//     copy of segment files if they are not on the same disk since hardlinks do
	//     not work across disks.
	...
	//     across all shards that will be shrunk into a single shard on the target
	//     index as this is the maximum number of docs that can fit into a single
	//     shard.
	...
	// `number_of_routing_shards` set to 30 (5 x 2 x 3) could be split by a factor
	// of 2 or 3.
	//
	// A split operation:
	//
	...
	//   - Hard-links segments from the source index into the target index. If the
	//     file system doesn't support hard-linking, all segments are copied into
...
type Ml struct {
	...
	// interval (`frequency`). If you are concerned about delayed data, you can add
	// a delay (`query_delay') at each interval. By default, the datafeed uses the
	// following query: `{"match_all": {"boost": 1}}`.
	//
...
type Security struct {
	...
	// `SAMLRequest`, which contains a SAML Authentication request that is deflated
	// and Base64 encoded. If the configuration dictates that SAML authentication
	// requests should be signed, the URL has two extra parameters named `SigAlg`
	// and `Signature`. These parameters contain the algorithm used for the
	// signature and the signature value itself. It also returns a random string
...
type Snapshot struct {
	...
	// reach a `blob_count` of at least `2000`, a `max_blob_size` of at least `2gb`,
	// a `max_total_data_size` of at least `1tb`, and a `register_operation_count`
	// of at least `100`. Always specify a generous timeout, possibly `1h` or
	// longer, to allow time for each analysis to run to completion. Perform the
	// analyses using a multi-node cluster of a similar size to your production
	...
	//     has been written, its contents must remain unchanged until it is
	//     deliberately modified or deleted.
	//   - Your repository must behave correctly even if connectivity from the
	//     cluster is disrupted. Reads and writes may fail in this case, but they
	//     must not return incorrect results.
	//
	...
	// parameters. If any of these reads fails then the repository does not
	// implement the necessary read-after-write semantics that Elasticsearch
	// requires.
	//
...
type TextStructure struct {
	...
	// ingested data into a very simple index that has just `@timestamp` and message
	// fields, you can use this API to see what common structure exists in the
	// message field.
	//
