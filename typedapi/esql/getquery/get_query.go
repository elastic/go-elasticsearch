...
// NewGetQueryFunc returns a new instance of GetQuery with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewGetQueryFunc(tp elastictransport.Interface) NewGetQuery {
	return func(id string) *GetQuery {
		n := New(tp)
...
// Returns an object extended information about a running ES|QL query.
//
// [Elasticsearch] https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-esql-get-query
//
...
func (r GetQuery) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		...
		if reader := instrument.RecordRequestBody(ctx, "esql.get_query", r.raw); reader != nil {
			req.Body = reader
		}
	}
	...
	if err != nil {
		localErr := fmt.Errorf("an error happened during the GetQuery query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
func (r GetQuery) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the GetQuery query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Id The query ID
// API Name: id
func (r *GetQuery) _id(id string) *GetQuery {
	r.paramSet |= idMask
	r.id = id
