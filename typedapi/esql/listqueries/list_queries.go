...
// NewListQueries type alias for index.
type NewListQueries func() *ListQueries

// NewListQueriesFunc returns a new instance of ListQueries with the provided transport.
// Used in the index of the library this allows to retrieve every apis in once place.
func NewListQueriesFunc(tp elastictransport.Interface) NewListQueries {
	return func() *ListQueries {
		n := New(tp)
...
// ES|QL queries.
//
// [Elasticsearch] https://www.elastic.co/docs/api/doc/elasticsearch/v9/operation-esql-list-queries
//
...
func (r ListQueries) Perform(providedCtx context.Context) (*http.Response, error) {
	...
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.BeforeRequest(req, "esql.list_queries")
		if reader := instrument.RecordRequestBody(ctx, "esql.list_queries", r.raw); reader != nil {
			req.Body = reader
		}
	}
	res, err := r.transport.Perform(req)
	if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
		instrument.AfterRequest(req, "elasticsearch", "esql.list_queries")
	}
	if err != nil {
		localErr := fmt.Errorf("an error happened during the ListQueries query execution: %w", err)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
...
// Do runs the request through the transport, handle the response and returns a listqueries.Response
...
func (r ListQueries) IsSuccess(providedCtx context.Context) (bool, error) {
	...
	if res.StatusCode != 404 {
		err := fmt.Errorf("an error happened during the ListQueries query execution, status code: %d", res.StatusCode)
		if instrument, ok := r.instrument.(elastictransport.Instrumentation); ok {
			instrument.RecordError(ctx, err)
		}
		return false, err
...
// Header set a key, value pair in the ListQueries headers map.
func (r *ListQueries) Header(key, value string) *ListQueries {
