...
// `max_total_data_size` of at least `1tb`, and a `register_operation_count` of
// at least `100`. Always specify a generous timeout, possibly `1h` or longer,
// to allow time for each analysis to run to completion. Some repositories may
// behave correctly when accessed by a small number of Elasticsearch nodes but
...
// BlobCount The total number of blobs to write to the repository during the test. For
// realistic experiments, set this parameter to at least `2000`.
// API name: blob_count
...
// Concurrency The number of operations to run concurrently during the test. For realistic
...
// MaxBlobSize The maximum size of a blob to be written during the test. For realistic
// experiments, set this parameter to at least `2gb`.
...
// RarelyAbortWrites Indicates whether to rarely cancel writes before they complete. For realistic
// experiments, leave this parameter unset.
...
// ReadNodeCount The number of nodes on which to read a blob after writing. For realistic
// experiments, leave this parameter unset.
...
// Timeout The period of time to wait for the test to complete. If no response is
